  ...
  >
    ...
    <template v-slot:item.programTitle="{ item }">
      <span class="text-capitalize">{{
        item.programTitle ? item.programTitle : "N/A"
      }}</span>
    </template>
    <template v-slot:item.date="{ item }">
      {{ item.dateFrom ? formatDate(item.dateFrom) : "N/A" }} -
      {{ item.dateTo ? formatDate(item.dateTo) : "N/A" }}
    </template>
  ...
  methods: {
    add() {
      if (this.form.programTitle) {
        this.trainingsLocal = [...this.trainingsLocal, this.form];
        this.clearForm();
      }
    },
    ...
    remove(row) {
      ...
      this.trainingsLocal = this.trainingsLocal.filter(
