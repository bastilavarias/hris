...
const employeeService = {
  create: async ({
    ...
    let error = {};
    let message = "";
    const isEmployeeExists = await helper.checkIfExists(
      "employee",
      "employee_number",
    ...
    const createdGovernmentIssueId = await governmentIssueIdModel.create(
      profile.governmentIssueId
    );
    ...
    if (profile.citizenship.length > 0)
      profile.citizenship.map(
        async name => await profileModel.addCitizenship(createdProfileId, name)
      );
    const accountDefaultPassword = accountService.generateDefaultPassword(
      employeeNumber,
      profile.lastName
    );
    const createdAccountId = await accountModel.create(
      employeeNumber,
      accountService.encryptPassword(accountDefaultPassword)
    );
  ...
  generateEmployeeNumber: async () => {
    ...
    const employeeTableCount = parseInt(
      await helper.countTableRows("employee")
    );
    let lastPart = "";
    if (employeeTableCount >= 0 && employeeTableCount <= 9)
      lastPart = `000${employeeTableCount}`;
    if (employeeTableCount >= 10 && employeeTableCount <= 99)
      lastPart = `00${employeeTableCount}`;
    if (employeeTableCount >= 100 && employeeTableCount <= 999)
      lastPart = `0${employeeTableCount}`;
  ...
  search: async (option, value) => {
    ...
    return await employeeModel.search(
      options[option],
  ...
  update: async (
    employeeId,
    { departmentId, designationId, isFullTime, profile }
  ) => {
    let error = {};
    let message;
    ...
    await governmentIssueIdModel.update(
      gotRawProfile.governmentIssueIdId,
      profile.governmentIssueId
    );
    ...
    if (profile.citizenship.length > 0) {
      profile.citizenship.map(nation =>
        profileModel.addCitizenship(gotRawProfile.id, nation)
      );
    }
    await familyModel.deleteChildren(gotRawProfile.familyId);
    if (profile.family.children.length > 0) {
      profile.family.children.map(child =>
        familyModel.addChild(gotRawProfile.familyId, child)
      );
    }
    await educationModel.delete(gotRawProfile.id);
    if (profile.education.length > 0) {
      profile.education.map(details =>
        educationModel.create(gotRawProfile.id, details)
      );
    }
    await civilServiceEligibility.delete(gotRawProfile.id);
    if (profile.civilServiceEligibility.length > 0) {
      profile.civilServiceEligibility.map(details =>
        civilServiceEligibility.create(gotRawProfile.id, details)
      );
    }
    await workExperienceModel.delete(gotRawProfile.id);
    if (profile.workExperiences.length > 0) {
      profile.workExperiences.map(details =>
        workExperienceModel.create(gotRawProfile.id, details)
      );
    }
    await voluntaryWorkExperienceModel.delete(gotRawProfile.id);
    if (profile.voluntaryWorkExperiences.length > 0) {
      profile.voluntaryWorkExperiences.map(details =>
        voluntaryWorkExperienceModel.create(gotRawProfile.id, details)
      );
    }
    await trainingModel.delete(gotRawProfile.id);
    if (profile.trainings.length > 0) {
      profile.trainings.map(details =>
        trainingModel.create(gotRawProfile.id, details)
      );
    }
    ...
    if (profile.recognitions.length > 0) {
      profile.recognitions.map(name =>
        recognitionModel.create(gotRawProfile.id, name)
      );
    }
    await organizationModel.delete(gotRawProfile.id);
    if (profile.organizations.length > 0) {
      profile.organizations.map(name =>
        organizationModel.create(gotRawProfile.id, name)
      );
    }
    await referenceModel.delete(gotRawProfile.id);
    if (profile.references.length > 0) {
      profile.references.map(details =>
        referenceModel.create(gotRawProfile.id, details)
      );
    }
