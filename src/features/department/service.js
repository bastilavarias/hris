...
module.exports = {
  create: async ({ name, description, employeeId }) => {
    let error = {};
    let message = "";
    const isDepartmentExists = await helper.checkIfExists(
      "department",
      "name",
      name.toLowerCase()
    );
    ...
    const createdDepartmentId = await departmentModel.create({
      name,
      description,
    });
  ...
  update: async (departmentId, { name, description, employeeId }) => {
    let message = "";
    let error = {};
    const foundDepartment = await departmentModel.getSingleByName(
      name.toLowerCase()
    );
    if (
      Object.keys(foundDepartment).length > 0 &&
      departmentId !== parseInt(foundDepartment.id)
    ) {
      error.name = "Department name was already used.";
      return {
        message,
  ...
  search: async (option, value) => {
    ...
    return await departmentModel.search(
      options[option],
